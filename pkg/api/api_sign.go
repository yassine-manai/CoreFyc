...
//	@Param			type	query		string					false	"Type of sign: all, enabled, or deleted (default is 'all')"
//	@Param			extra	query		string					false	"Include extra information if 'yes'"
//	@Success		200		{object}	db.Sign					"List of signs or a single sign"
...
func GetSignAPI(c *gin.Context) {
	...
	idStr := c.Query("id")
	signType := c.DefaultQuery("type", "all") // 'all', 'enabled', or 'deleted'
	extraReq := c.DefaultQuery("extra", "false")

	// Check if ID is provided to fetch a single sign
	if idStr != "" {
		...
		// Fetch specific sign based on type
	...
	log.Info().Str("extra", extraReq).Str("type", signType).Msg("Fetching signs")

	var signs []db.SignResp
	var err error

	// Fetch list of signs based on type
	signs, err = db.GetAllSigns(ctx)

	if err != nil {
		log.Err(err).Msg("Error retrieving signs")
		c.JSON(http.StatusInternalServerError, gin.H{
			"error":   "An unexpected error occurred",
			"message": "Error retrieving signs",
			"code":    10,
		})
		return
	}
	...
	if len(signs) == 0 {
		log.Info().Str("type", signType).Msg("No signs found")
		c.JSON(http.StatusNotFound, gin.H{
			"error":   "Not Found",
	...
	log.Info().Int("sign_count", len(signs)).Str("type", signType).Msg("Signs fetched successfully")
...
//	@Accept			json
//	@Produce		json
//	@Param			sign	body		db.Sign					true	"sign data"
//	@Success		201		{object}	db.Sign					"sign created successfully"
...
//	@Param			sign	body		db.Sign					true	"Updated sign data"
...
func ChangeSigntateAPI(c *gin.Context) {
	...
	rowsAffected, err := db.ChangeSignState(ctx, id, state)
	if err != nil {
...
//	@Produce		json
//	@Param			id		query		int	false	"sign ID"
//	@Success		200		{object}	db.Sign		"List of enabled sign or a single sign"
//	@Failure		500		{object}	map[string]interface{}	"Internal server error"
//	@Failure		404		{object}	map[string]interface{}	"No sign found"
//	@Failure		400		{object}	map[string]interface{}	"Bad request: Invalid sign ID"
...
func GetSignEnabledAPI(c *gin.Context) {
	...
	if idStr != "" {
		...
		sign, err := db.GetEnabledSignByID(ctx, id)
		if err != nil {
			log.Err(err).Str("sign_id", idStr).Msg("Error retrieving sign by ID")
			c.JSON(http.StatusNotFound, gin.H{
				"error":   "Not Found",
	...
	sign, err := db.GetSignListEnabled(ctx)
	if err != nil {
